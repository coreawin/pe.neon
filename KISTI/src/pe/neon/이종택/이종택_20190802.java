...
public class 이종택_20190802 {
	...
	private String[] retrievePnoList(Set<String> pnoList) {
		...
		for (String au : ks) {
			...
			System.out.println("REFERENCE INFO " + au + "====> " + col.count(doc));
		...
		try {
			...
			for (String datas : set) {
				...
				if (cnt % 500 == 0) {
					...
					w4.flush();
					w5.flush();
					System.out.println("progress : " + cnt + "/"+totalCnt);
					_tmp.clear();
				}
				cnt++;
			}
			if(_tmp.size()>0){
				...
				w4.flush();
				w5.flush();
				System.out.println("progress : " + (cnt + _tmp.size()) + "/+totalCnt");
				_tmp.clear();
			}
	...
	public void testWriteParseFromMongoDB() throws Exception {
		System.out.println("20180721  이종택 mission");
		System.setProperty("DEBUG.MONGO", "false");
		mc = MongoDBConnector.getInstance("203.250.207.75", 27017);
		db = mc.getDatabase("KISTI_2017_PATENT");
		MongoCollection<Document> collection = db.getCollection("US");
		// DBCollection col = db.getCollection("US");
		BasicDBObject doc = new BasicDBObject("pyear", "2018");
		FindIterable<Document> fIter = collection.find(Filters.eq("pyear", "2018"));
		...
		MongoCursor<Document> mCur = fIter.iterator();
		// int total = cur.count();
		long total = collection.count(Filters.eq("pyear", "2018"));

		System.out.println("total : " + total);
		parser = new PatentJAXBParser();
		PatentTableData ptd = PatentTableData.getInstance();
		DivisionFileWriter w1 = new DivisionFileWriter("d:\\data\\us.grant.2017.backward.txt");
		DivisionFileWriter w2 = new DivisionFileWriter("d:\\data\\us.grant.2017.ipc.txt");
		DivisionFileWriter w3 = new DivisionFileWriter("d:\\data\\us.grant.2017.publication.txt");
		DivisionFileWriter w4 = new DivisionFileWriter("d:\\data\\us.grant.2017.ref.ipc.txt");
		DivisionFileWriter w5 = new DivisionFileWriter("d:\\data\\us.grant.2017.ref.publication.txt");
		int cnt = 0;
		int grantCnt = 0;
		while (mCur.hasNext()) {
			...
			try {
				...
				if ("grant".equalsIgnoreCase(publ_type)) {
					...
					// System.out.println(pno +"\t" + ptd.citations.toString());
					...
					// System.out.println(ptd.citations.citedList.toString());
					// for(String s : sets){
					// System.out.println("backwardSet " + s);
					// // w2.write(pno +"\t" + s +"\n");
					// }
					//
					...
					// System.out.println(ptd.application.lineData());
					// System.out.println(ptd.title.lineData());
					if (grantCnt % 1000 == 0) {
						System.out.println("progress grantCnt(332,483) : " + grantCnt + "/332,483");
					}
					grantCnt++;
				}
				if (cnt % 1000 == 0) {
					System.out.println("progress : " + cnt + "/" + total);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
