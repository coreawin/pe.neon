...
import kr.co.tqk.web.db.dao.export.*;
...
public class DownloadSearchUtil {
    ...
    public boolean execute() throws Exception {
        try {
            DescriptionCodeManager.getInstance();
            logger.info("30초간 기다립니다.");
...
//					logger.info("download documents {}/{}", nf.format(progress), nf.format(totalCnt));
                ...
                if (isContinues) {
                    ...
                    logger.info("download documents  {}", nf.format(last) + "/" + nf.format(totalCnt));
                }
            ...
            logger.info("download Count Info : {}", totalCnt);
            logger.info("download RESULT COUNT Info : {}", totalSearchCnt);
            isSuccess = true;
            return isSuccess;
        } catch (Exception e) {
