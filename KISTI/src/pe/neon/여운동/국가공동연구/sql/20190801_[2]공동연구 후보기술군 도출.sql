...
create TABLE NYEO2019_SCOPUS_AKMYDATA NOLOGGING AS
    SELECT keyword, publication_year, count(distinct eid) AS "키워드발생건수" FROM NYEO2019_SCOPUS_FILTERING_DOC
...
/*
    @coreawin 201908 작업 : 최근 5년간의 논문 ID, 논문의 연도, 소분류, 국가 정보 수집 쿼리
    - 국가코드는 중복을 포함하여 수집
	- 논문의 Fractional Count 방식의 피인용수와 논문건수를 계산할떄 실제 논문의 국가의 총건수, 논문의 국가별 건수 수집을 위해 사용.
*/
DROP TABLE NYEO2019_FILTER_AFF_CT_EID CASCADE CONSTRAINTS PURGE;
CREATE TABLE NYEO2019_FILTER_AFF_CT_EID NOLOGGING AS
SELECT /*+ PARALLEL (4) */  DOC_INFO.EID, DOC_INFO.PUBLICATION_YEAR, ASJC.ASJC_CODE, DECODE(NVL(AFF_INFO.COUNTRY_CODE, 'NONE'), '.', 'NONE', NVL(AFF_INFO.COUNTRY_CODE, 'NONE')) AS COUNTRY_CODE
FROM NYEO2019_SCOPUS_FILTERING_DOC DOC_INFO
LEFT OUTER JOIN(
...
LEFT OUTER JOIN (
  SELECT DISTINCT EID, DECODE(ASJC_CODE, NULL, 'NONE',ASJC_CODE) AS ASJC_CODE
  FROM SCOPUS_2014_CLASS_ASJC
) ASJC
ON DOC_INFO.EID = ASJC.EID;



COMMENT ON TABLE NYEO2019_FILTER_AFF_CT_EID IS '@coreawin 201908 작업 :[FILTERING]  논문 ID, 논문의 연도, 대분류, 국가 정보 수집 쿼리. 국가코드 중복 허용 수집, 논문의 Fractional Count 방식의 피인용수와 논문건수를 계산할떄 실제 논문의 국가의 총건수, 논문의 국가별 건수 수집을 위해 사용.';
COMMENT ON COLUMN NYEO2019_FILTER_AFF_CT_EID.EID IS '문헌 EID';
COMMENT ON COLUMN NYEO2019_FILTER_AFF_CT_EID.PUBLICATION_YEAR IS '발행연도';
COMMENT ON COLUMN NYEO2019_FILTER_AFF_CT_EID.ASJC_CODE IS 'ASJC 대분류 코드';
COMMENT ON COLUMN NYEO2019_FILTER_AFF_CT_EID.COUNTRY_CODE IS '국가코드';

CREATE INDEX IDX_FILTERING_AFFCT_CT ON NYEO2019_FILTER_AFF_CT_EID(COUNTRY_CODE) NOLOGGING parallel 2;
CREATE INDEX IDX_FILTERING_AFFCT_EID ON NYEO2019_FILTER_AFF_CT_EID(EID) NOLOGGING parallel 2;
CREATE INDEX IDX_FILTERING_AFFCT_YEAR ON NYEO2019_FILTER_AFF_CT_EID(PUBLICATION_YEAR) NOLOGGING parallel 2;
CREATE INDEX IDX_FILTERING_AFFCT_LASJC ON NYEO2019_FILTER_AFF_CT_EID(ASJC_CODE) NOLOGGING parallel 2;